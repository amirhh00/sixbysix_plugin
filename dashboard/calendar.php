  ...
  <div class="wrap">
    ...
    <div class="max-md:!grid grid-rows-2 grid-cols-2 *:transition-all" style="display: flex; gap: 16px; justify-content: space-between; width: 100%;">
      ...
      <div class="flex max-md:flex-col md:gap-2 col-span-2 w-min mx-auto max-md:order-first justify-center items-center">
        ...
        <button class="btn hover:*:fill-white !p-1 !min-w-0 aspect-square" style="border: none;" id="reloadMonth">
          <svg xmlns="http://www.w3.org/2000/svg" style="width: 30px; height:30px;" class="fill-black" viewBox="0 0 512 512">
        ...
        fetch(`${REST_API_BASE}/calendar?month=${MonthWithPad}&year=${year}`)
          ...
          .then(
            ...
            (allEventsForMonth) => {
              ...
              calendar.style.backgroundImage = allEventsForMonth.monthBg ? `url(${allEventsForMonth.monthBg})` : 'url(https://placehold.co/600x400@3x/000000/000?text=img)';
...
function sixonesix_set_event_for_day(WP_REST_Request $request)
{
  ...
  if ($events === '' || count($events) === 0) {
    ...
    update_option('sixonesix_calendar_' . $formattedDate, $dayobject);
  }
...
function sixonesix_get_calendar_data(WP_REST_Request $request)
{
  ...
  for ($i = 1; $i <= $daysInMonth; $i++) {
    ...
    $dayobjectValue = get_option('sixonesix_calendar_' . $currentDay->format('Y-m-d'));
    $calendarDays[$currentDay->format('Y-m-d')] = $dayobjectValue ? $dayobjectValue : null;
  }
  // calendarDays in in calendarData object
  $calendarData = array(
    'monthBg' => get_option('sixonesix_calendar_background_' . $month . '-' . $year),
    'days' => $calendarDays
  );
